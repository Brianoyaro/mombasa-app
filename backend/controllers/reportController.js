...
exports.createReport = async (req, res) => {
  ...
  try {
    const [result] = await db.execute(
      'INSERT INTO reports (user_id, title, description, location, department_id) VALUES (?, ?, ?, ?, ?)',
...
exports.getAllReportsWithVotesAndImages = async (req, res) => {
  try {
    const [reports] = await db.execute(
      `SELECT r.*, 
        (SELECT COUNT(*) FROM votes WHERE report_id = r.id AND vote_type = 'up') as upvotes,
        (SELECT COUNT(*) FROM votes WHERE report_id = r.id AND vote_type = 'down') as downvotes,
        (SELECT JSON_ARRAYAGG(image_url) FROM report_images WHERE report_id = r.id) as images
      FROM reports r`
    );
    res.json(reports);
  } catch (error) {
...
exports.updateReport = async (req, res) => {
  ...
  try {
    await db.execute(
      'UPDATE reports SET title = ?, description = ?, location = ?, status = ? WHERE id = ?',
